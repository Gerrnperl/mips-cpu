...
export function readable(result) {
    ...
    const symbolTableReversed = new Map();
    ...
    for (let i = 0; i < binary.length; i++) {
        ...
        lines.push(`${int32ToHex8(lineNum << 2)}: ${int32ToHex8(binary[i])}  ${binaryLine}  ${line}`);
        lineNum += line.split("\n").length;
    }
    return lines.join("\n");
}
function toReadableBinary(binary) {
